# -*- coding: utf-8 -*-
"""
Created on Tue Oct 13 22:10:00 2020
...
def buildFeatureVectorForScoring2(hits, gRNA_size=20, canonical_PAM = "NGG", PAM_size = 3, PAM_location = "3prime"):
    ...
    for j in range(len(subject)):
        PAM.append(subject.iloc[j,1][gRNA_size+1:])
    print(PAM)
    mismatches = []
    ...
    arr_ind = pd.concat([pd.DataFrame(arr_ind[0],index=range(len(arr_ind[0])),columns=["row"]),pd.DataFrame(arr_ind[1],index=range(len(arr_ind[0])),columns=["col"])], axis=1)
    ind_row = arr_ind["row"]
    ...
    partitioningstart = []
    partitioningend = []
    for i in range(1,max(list(ind_row))+1):
        if i in list(ind_row): 
            partitioningwidth.append(list(ind_row).count(ind_row[i]))
            partitioningstart.append(s)
            e = list(ind_row).count(ind_row[i]) + s - 1
            s = e + 1
    ...
    for i in range(np.shape(mismatch_pos)[0]):
        if mismatch_pos.iloc[i,0] == "-":
            ...
            mean_neighbor_distance_mismatch.append(int(mismatch_pos.iloc[i,1]) - int(mismatch_pos.iloc[i,0]))
    ...
    for i in range(np.shape(features)[0]):
        if features["mismatch.distance2PAM"].iloc[i] == ["-", "-"]:
            features["mismatch.distance2PAM"].iloc[i] = ""
